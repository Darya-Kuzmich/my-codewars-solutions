...
# Examples:
# must return 2
# cakes({flour: 500, sugar: 200, eggs: 1}, {flour: 1200, sugar: 1200, eggs: 5, milk: 200})  # noqa
# # must return 0
# cakes({apples: 3, flour: 300, sugar: 150, milk: 100, oil: 100}, {sugar: 500, flour: 2000, milk: 2000})  # noqa

def cakes(recipe: dict, available: dict) -> int:
    if not set(recipe).issubset(set(available)):
        return 0
    amount = []
    for ingredient in set(recipe).intersection(set(available)):
        amount.append(available[ingredient] // recipe[ingredient])
    return min(amount)
